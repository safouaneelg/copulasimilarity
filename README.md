...
```mermaid
flowchart TD
    A[Input Images] --> B{Extract Local Features}
    B -->|Patches| C[Reshape Features]
    C --> D[Compute Ranks]
    D --> E[Apply PPF]
    E --> F[Copula Vectors]
    F --> G[Euclidean Distance]
    G --> H[Local Similarities]
    H --> I{More Patches?}
    I -->|Yes| C
    I -->|No| J[Reshape Similarities]
    J --> K[Output Similarity Map]
```
...
sequenceDiagram
    ...
    participant EL as Extract Local Features
    participant RF as Reshape Features
    participant CR as Compute Ranks
    participant PPF as Apply PPF
    participant CV as Copula Vectors
    participant ED as Euclidean Distance
    participant LS as Local Similarities
    participant RS as Reshape Similarities
    participant OSM as Output Similarity Map

    I->>EL: Images
    EL->>RF: Patches
    RF->>CR: Feature Vectors
    CR->>PPF: Ranks
    PPF->>CV: Copula Vectors
    CV->>ED: Copula Vectors
    ED->>LS: Similarity
    LS->>RS: Local Similarities
    RS->>OSM: Reshaped Similarities
```

```mermaid
stateDiagram
    [*] --> Input
    Input --> ExtractLocalFeatures: Images
    ExtractLocalFeatures --> ReshapeFeatures: Patches
    ReshapeFeatures --> ComputeRanks: Feature Vectors
    ComputeRanks --> ApplyPPF: Ranks
    ApplyPPF --> CopulaVectors: Copula Vectors
    CopulaVectors --> EuclideanDistance: Copula Vectors
    EuclideanDistance --> LocalSimilarities: Similarity
    LocalSimilarities --> ReshapeSimilarities: Local Similarities
    ReshapeSimilarities --> OutputSimilarityMap: Reshaped Similarities
    OutputSimilarityMap --> [*]
```
...
gantt
    ...
    title Copula-Based Similarity Method
    section Extract Features
        Extract Local Features  :a1, 2024-01-01, 2 days
        Reshape Features        :after a1, 2 days
    section Compute Copula
        Compute Ranks           :a2, after a1, 1 day
        Apply PPF               :after a2, 1 day
        Copula Vectors          :after a3, 1 day
    section Compute Similarity
        Euclidean Distance      :a4, after a3, 1 day
        Local Similarities      :after a4, 1 day
        Reshape Similarities    :after a5, 1 day
    section Output
        Output Similarity Map   :a6, after a5, 1 day
```
## 🌟 Features
