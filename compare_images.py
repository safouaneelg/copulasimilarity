...
def main():
    ...
    parser.add_argument('--save_csim_map', action='store_true', help='Save the Copula-Based Similarity Map')
    ...
    copula_similarity = CSIMSimilarity(patch_size=args.patch_size)  
    ...
    print(f"CSIM: {csim:.4f}")
    
    if args.save_csim_map:
        plt.figure(figsize=(8, 6))
        plt.title('Copula-Based Similarity Map')
        plt.imshow(csim_map, cmap='hot', interpolation='nearest')
        plt.colorbar(label='Similarity')
        plt.savefig('csim_map.png')
        print("CSIM Map saved as 'csim_map.png'")
